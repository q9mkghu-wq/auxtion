...
export default function CourtPage() {
  ...
  const handleSearch = () => fetchData(1, keyword);
  ...
  const inputStyle = { flex: 1, minWidth: "220px", padding: "12px 14px", borderRadius: "12px", border: "1px solid #cbd5e1", fontSize: "14px", boxSizing: "border-box" };
    ...
    <main style={{ minHeight: "100vh", background: "#f8fafc", padding: "24px", fontFamily: "Arial, sans-serif" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <section style={{ background: "linear-gradient(135deg, #0f172a, #1e3a8a)", color: "#ffffff", borderRadius: "20px", padding: "28px", marginBottom: "24px" }}>
          <h1 style={{ margin: 0, fontSize: "32px", fontWeight: 700 }}>{"법원경매 수집 테스트"}</h1>
          <p style={{ marginTop: "10px", fontSize: "15px", lineHeight: 1.6, opacity: 0.95 }}>{"지역 키워드로 매각예정물건을 바로 필터링할 수 있어요."}</p>
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginTop: "18px" }}>
            <input value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} placeholder="예: 서울, 부산, 평창동" style={inputStyle} />
            <button onClick={handleSearch} style={{ ...buttonStyle, background: "#ffffff", color: "#0f172a" }}>{"지역 검색"}</button>
            <button onClick={() => { setKeyword(""); fetchData(1, ""); }} style={{ ...buttonStyle, background: "#111827", color: "#ffffff" }}>{"전체 보기"}</button>
          </div>
        </section>
        ...
        <section style={{ ...cardStyle, marginBottom: "20px" }}>
          <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
            ...
            <div><div style={labelStyle}>{"현재 표시 건수"}</div><div style={valueStyle}>{data.count}{"건"}</div></div>
          </div>
        </section>
            ...
            {data.items.map((item, index) => (
              <article key={item.docid + "-" + index} style={cardStyle}>
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  style={{ display: "block", padding: "10px", background: "#1976D2", color: "#fff", borderRadius: "10px", textAlign: "center", fontWeight: 700, fontSize: "14px", textDecoration: "none" }}
                >{"🔗 법원경매 바로가기"}</a>
              </article>
            ))}
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "12px", marginTop: "24px", flexWrap: "wrap" }}>
          <button onClick={() => { if (page > 1) fetchData(page - 1, appliedRegion); }} disabled={loading || page <= 1} style={{ ...buttonStyle, background: page <= 1 ? "#cbd5e1" : "#e5e7eb", color: "#111827", cursor: page <= 1 ? "not-allowed" : "pointer" }}>{"이전 페이지"}</button>
          <button onClick={() => fetchData(page + 1, appliedRegion)} disabled={loading} style={{ ...buttonStyle, background: "#4f46e5", color: "#ffffff" }}>{"다음 페이지"}</button>
        </div>
      </div>
    </main>
  );
